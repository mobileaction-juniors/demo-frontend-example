  ...
  >
    ...
    >
      ...
      >
        ...
        <tbody>
        <tr
            v-for="(keywords, count) in groupedKeywordCounts"
            :key="count"
        >
          <td
              class="border px-4 py-2"
    ...
    <button
        @click="resetData"
        class="mt-2 bg-blue-500 hover:bg-red-400-600 text-white px-4 py-2 rounded"
    >
      Reset Data
    </button>
    <div
...
const groupedKeywordCounts = computed(() => {
  const counts = {};
  const words = textInput.value.split(/\s+/).filter(word => word.trim() !== '').map(word => word.toLowerCase().trim());
